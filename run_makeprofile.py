...
samples  = ['original_bin1','original_bin2','original_bin3','original_bin4']*4
...
name_cat = name_cat + ['gx_CFHT_redMapper.fits']*4 + ['gx_CS82_redMapper.fits']*4 + ['gx_KiDS_redMapper.fits']*4
samples  = samples +  ['terciles_total','terciles_bin1','terciles_bin2','terciles_bin3']*4
lmin     = np.append(lmin,np.tile(np.array([20.,20.,28.,40.]),3))
lmax     = np.append(lmax,np.tile(np.array([150.,28.,40.,150.]),3))
zmin     = np.append(zmin,np.ones(len(lmin))*0.1)
zmax     = np.append(zmax,np.ones(len(lmin))*0.33)
z_back   = np.append(z_back,np.ones(len(lmin))*0.1)
odds_min = np.append(odds_min,np.ones(len(lmin))*0.5)
RIN      = np.append(RIN,np.ones(len(lmin))*200.)
ROUT     = np.append(ROUT,np.ones(len(lmin))*5000.)
ndots    = np.append(ndots,np.ones(len(lmin))*10)
